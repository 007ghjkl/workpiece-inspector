...
private slots:
    void invalidUrlFailsClearly()
    {
        ...
        QVERIFY(QString::fromLatin1(source.status().message).contains(QStringLiteral("URL")));
    ...
    void wrongModeFailsClearly()
    {
        ...
        QVERIFY(source.status().state == ImageSourceState::Faulted);
        QVERIFY(QString::fromLatin1(source.status().message).contains(QStringLiteral("mode")));
    }
